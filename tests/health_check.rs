...
// Ensure that the `tracing` stack is only initialized once using `lazy_static`
...
async fn spawn_app() -> TestApp {
    ...
    // We return the TestApp object to the caller
    TestApp {
        address,
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    // Create database
    let mut connection = PgConnection::connect(&config.connection_string_with_no_db())
        .await
